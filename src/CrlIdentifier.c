/*
 * Generated by asn1c-0.9.22 (http://lionet.info/asn1c)
 * From ASN.1 module "ETS-ElectronicSignatureFormats-88syntax"
 * 	found in "lpa2.asn1"
 * 	`asn1c -S/skeletons`
 */

#include <asn_internal.h>

#include "CrlIdentifier.h"

static asn_TYPE_member_t asn_MBR_CrlIdentifier_1[] = {
	{ ATF_NOFLAGS, 0, offsetof(struct CrlIdentifier, crlissuer),
		-1 /* Ambiguous tag (CHOICE?) */,
		0,
		&asn_DEF_Name,
		0,	/* Defer constraints checking to the member type */
		0,	/* PER is not compiled, use -gen-PER */
		0,
		"crlissuer"
		},
	{ ATF_NOFLAGS, 0, offsetof(struct CrlIdentifier, crlIssuedTime),
		(ASN_TAG_CLASS_UNIVERSAL | (23 << 2)),
		0,
		&asn_DEF_UTCTime,
		0,	/* Defer constraints checking to the member type */
		0,	/* PER is not compiled, use -gen-PER */
		0,
		"crlIssuedTime"
		},
	{ ATF_POINTER, 1, offsetof(struct CrlIdentifier, crlNumber),
		(ASN_TAG_CLASS_UNIVERSAL | (2 << 2)),
		0,
		&asn_DEF_NativeInteger,
		0,	/* Defer constraints checking to the member type */
		0,	/* PER is not compiled, use -gen-PER */
		0,
		"crlNumber"
		},
};
static ber_tlv_tag_t asn_DEF_CrlIdentifier_tags_1[] = {
	(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))
};
static asn_TYPE_tag2member_t asn_MAP_CrlIdentifier_tag2el_1[] = {
    { (ASN_TAG_CLASS_UNIVERSAL | (2 << 2)), 2, 0, 0 }, /* crlNumber at 270 */
    { (ASN_TAG_CLASS_UNIVERSAL | (16 << 2)), 0, 0, 0 }, /* rdnSequence at 544 */
    { (ASN_TAG_CLASS_UNIVERSAL | (23 << 2)), 1, 0, 0 } /* crlIssuedTime at 269 */
};
static asn_SEQUENCE_specifics_t asn_SPC_CrlIdentifier_specs_1 = {
	sizeof(struct CrlIdentifier),
	offsetof(struct CrlIdentifier, _asn_ctx),
	asn_MAP_CrlIdentifier_tag2el_1,
	3,	/* Count of tags in the map */
	0, 0, 0,	/* Optional elements (not needed) */
	-1,	/* Start extensions */
	-1	/* Stop extensions */
};
asn_TYPE_descriptor_t asn_DEF_CrlIdentifier = {
	"CrlIdentifier",
	"CrlIdentifier",
	SEQUENCE_free,
	SEQUENCE_print,
	SEQUENCE_constraint,
	SEQUENCE_decode_ber,
	SEQUENCE_encode_der,
	SEQUENCE_decode_xer,
	SEQUENCE_encode_xer,
	0, 0,	/* No PER support, use "-gen-PER" to enable */
	0,	/* Use generic outmost tag fetcher */
	asn_DEF_CrlIdentifier_tags_1,
	sizeof(asn_DEF_CrlIdentifier_tags_1)
		/sizeof(asn_DEF_CrlIdentifier_tags_1[0]), /* 1 */
	asn_DEF_CrlIdentifier_tags_1,	/* Same as above */
	sizeof(asn_DEF_CrlIdentifier_tags_1)
		/sizeof(asn_DEF_CrlIdentifier_tags_1[0]), /* 1 */
	0,	/* No PER visible constraints */
	asn_MBR_CrlIdentifier_1,
	3,	/* Elements count */
	&asn_SPC_CrlIdentifier_specs_1	/* Additional specs */
};

